...
import { Play, Users, Zap, Star, Crown, ExternalLink } from 'lucide-react'
...
interface GameCardProps {
  ...
  onPopout?: (game: Game) => void
}

const GameCard: React.FC<GameCardProps> = ({ game, variant = 'default', onClick, onPlay, onPopout }) => {
  ...
  if (variant === 'compact') {
    return (
      <motion.div
        ...
        onClick={onClick}
      >
        <div className="relative aspect-square bg-gradient-to-br from-gray-800 to-gray-900 rounded-lg overflow-hidden border border-gray-700 hover:border-[#00d4ff]/50 transition-all duration-300">
          {/* Background Image */}
          ...
          <div className="absolute top-1 left-1 flex flex-col space-y-0.5">
            {game.isExclusive && (
              <span className="bg-[#00d4ff] text-black text-[10px] px-1.5 py-0.5 rounded-full font-bold">
                EXCLUSIVE
              </span>
            )}
            {game.isNew && (
                  ...
                  <div className="flex items-center space-x-2">
                    <Link href={game.provider === 'EDGE ORIGINALS' ? `/originals/${game.id}` : `/casino/game/${game.id}`}>
                      <Button 
                        variant="default" 
                        size="sm" 
                        className="bg-[#00d4ff] text-black text-xs px-2 py-1"
                    ...
                    {onPopout && (
                      <Button 
                        variant="ghost" 
                        size="sm" 
                        className="bg-gray-800/80 text-white text-xs px-2 py-1 hover:bg-gray-700/80"
                        onClick={(e) => {
                          e.stopPropagation()
                          onPopout(game)
                        }}
          ...
          {/* Player Count */}
          <div className="absolute bottom-1 right-1 bg-black/70 text-white text-[10px] px-1.5 py-0.5 rounded-full flex items-center">
            <Users className="h-2.5 w-2.5 mr-0.5 text-green-400" />
